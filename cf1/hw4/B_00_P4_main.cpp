...
//       GROUP:  B
// DESCRIPTION:  Programs 4 for this class will serve as an 
...
//*****PROGRAM 4 ADDITION*****
		else if (choice.compare("ftree") = 0)
		{
			cin >> choice1;
			pos = searchID(head, choice1);
...
void fTree(list* head, list* pos)
{
//create a string array to hold all of the print data
	string gen[5];
//create a temporary pointer to pass through each member of the tree
	list* temp = new list;
//create counter array for grandchildren
	int numGrand[pos->numChil];

//initialize this array so that we can add to it
	for (int i=0; i<5; i++)
	{
		gen[i] = "";
	}

//add the root of the tree to the first line	
	sprintf(gen[0], "%.18s", pos->name);

//check if there is a spouse (if so, add to first line)
	if (pos->spouse != 0)
	{
		sprintf(gen[0], " -- %.18s", (pos->spouse)->name);
	}

//gen[1] is the first buffer line
	gen[1] += "|";

//**gen[2] is populated by the children of the root, 
//**taking into consideration space needed for gen[4] (3rd generation)

//check how many children each node has
	if (pos->numChil > 0)
	{
//for each child, populate their subtree
		for (int i=0; i<pos->numChil; i++)
		{
//add horizontal buffer between siblings
			if (i>0)
			{
				sprintf(gen[2], "    ||");
			}
			...
			temp = pos->child[i];
//add child
			sprintf(gen[2], "%.18s", temp->name);

//check if there is a spouse (if so, add to line)
			if (temp->spouse != 0)
			{
				sprintf(gen[2], " -- %.18s", (temp->spouse)->name);
			}
				
//add grandkids
			if (temp->numChil > 0)
			{
//add to buffer (size for parents)
				sprintf(gen[3], "%.40s", "|");

				if (temp->numChil == 3)
				{
//additional buffer for 3rd child
					sprintf(gen[3], "%.22s", "");
				}

				for (int j=0; j<temp->numChil; j++)
				{
					sprintf(gen[4], "%.22s", (temp->child[j])->name);
				}
			}
		}
	}

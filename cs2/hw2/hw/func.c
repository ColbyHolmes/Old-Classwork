...
/*Set the number of grades the course will have*/
	printf("How many grades will this class have: ");
...
/*assign student id # and set the # of classes to 0*/
	...
	students[i].cNum = 4;
/*dyn allocate enrollment*/
	students[i].enroll = (enrollment*) malloc(sizeof((students[i]).enroll)*students[i].cNum);
...
/*scan in and check student id*/
	...
	else if(students[i].classes == students[i].cNum)
	{
		students[i].cNum +=4;
		students[i].enroll = (enrollment*) realloc(students[i].enroll, students[i].cNum);
	}
...
/*open the gradebook data file and store the student and course arrays and their counts*/
	...
	fwrite(students, sizeof(student), sCount, out);
	fwrite(courses, sizeof(course), cCount, out);
	fclose(out);
...
/*open the file and return to the top, then read the data into their respective arrays*/
	...
	fread(students, sizeof(student), *sCount, in);
	fread(courses, sizeof(course), *cCount, in);
	fclose(in);

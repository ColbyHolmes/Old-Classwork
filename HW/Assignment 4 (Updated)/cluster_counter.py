...
diag1 = ' '#bandaid (didnt check for diagonal matching before...)
#i tried checking upper right, but failed, SO this code isnt perfect.
#IT will miss connections via upper right diagonal.
...
#check if previous element is different
	    ...
	    diag1 = lst[count]
            ...
            count += 1
            continue
#check diag for difference
	  elif token.lower() == diag1.lower():
	    diag1 = lst[count]
            lst[count] = token
            count += 1
            continue
...
#replace lst with current line token
      diag1 = lst[count] #bandaid for upper left diagonal checking
      lst[count] = token
      count += 1
    #end token loop
